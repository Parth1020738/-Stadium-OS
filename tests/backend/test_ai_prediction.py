...
from backend.app.services.ai_decision_service import RiskPredictionService

test_db_url = "sqlite+aiosqlite:///./test_ai_pred.db"
...
async def setup_db():
    ...
    if os.path.exists("./test_ai_pred.db"):
        try:
            os.remove("./test_ai_pred.db")
        except Exception:
            pass
...
@pytest.mark.asyncio
async def test_risk_prediction_calculation(setup_db):
    async_session = setup_db
    async with async_session() as session:
        service = RiskPredictionService(session)
        risk = await service.calculate_live_risk()
        assert risk.overall_risk > 30.0
        assert risk.status in ["LOW", "MEDIUM", "HIGH", "CRITICAL"]
        assert risk.crowd_risk == 85.0
